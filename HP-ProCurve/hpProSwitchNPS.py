...
def copyConfigs(usernameDecode, passwordDecode):
    ...
    for host in iplist:
        ...
        try:
            ...
            net_connect.send_command_timing("radius-server host 10.91.1.22 key RadiusMoment*", strip_command=False, strip_prompt=False)        
            
            print("> Setup Server Host 2") 
            net_connect.send_command_timing("radius-server host 10.91.1.19 key RadiusMoment*", strip_command=False, strip_prompt=False)
            ...
            net_connect.send_command_timing("aaa server-group radius nps-servers host 10.91.1.22", strip_command=False, strip_prompt=False)
            
            print("> Setup nps-server group with host 2")  
            net_connect.send_command_timing("aaa server-group radius nps-servers host 10.91.1.19", strip_command=False, strip_prompt=False)
